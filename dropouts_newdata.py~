...
class AutoEncoder(object):
    ...
    def train(self, n_epochs=100, mini_batch_size=1, learning_rate=0.1):
        ...
        output = T.dot(hidden_tilde,T.transpose(self.W))+self.b2
        ...
        for epoch in xrange(n_epochs):
            ...
            for row in xrange(0,self.m, mini_batch_size):
                adding= adding + train(row)[0]
                #print parak
                # print train(row)[0]
	    global_list.append(adding/mini_batch_size)
...
def load_data(dataset):
    ...
    mat = sio.loadmat('../RetinalFundusImagesData.mat')
    data = mat['DataMatrix']
...
def plot_first_k_numbers(X,k):
    ...
    fig.show()
    fig.savefig("dropout_new.jpg")
    

def m_test(data):
    X=data[0]
    #plot_first_k_numbers(X, 100)
    activation_function = T.nnet.sigmoid
    output_function=activation_function
    A = AutoEncoder(X, 700, activation_function, output_function)
    A.train(50,100)
