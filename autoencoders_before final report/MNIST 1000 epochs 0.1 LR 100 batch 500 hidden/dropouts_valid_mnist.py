...
def plot_first_k_numbers(X,k):
    ...
    pyplot.tick_params(\
        ...
        labelleft='off')
    fig.savefig("dec12_mnist.jpg")
    fig.show()

def m_test(data):
    X=data[0][0]# train set
    Y=data[1][0]# valid set
    activation_function = T.nnet.sigmoid
    output_function=activation_function
    A = AutoEncoder(X,Y, 500, activation_function, output_function) # 500 hidden layers
    A.train(1000,100) #1000 epochs
    W=np.transpose(A.get_weights()[0])
    plot_first_k_numbers(W, 100)
    #write the weight matrix both to csv and mat
    import csv
    with open('dec12_mnist.csv', 'w') as fp:
        a = csv.writer(fp, delimiter=',')
        a.writerows(W)

    import scipy.io as sio
    sio.savemat('dec12_mnist.mat',{'weights':W})
 
m_test(data)
import matplotlib.pyplot as plt
# plots both valid and test costs
plt.figure()
...
plt.savefig("dec12_cost_mnist.jpg")
plt.show()
