...
class AutoEncoder(object):
    ...
    def train(self, n_epochs=100, mini_batch_size=1, learning_rate=0.02):
        index = T.lscalar()
        x=T.matrix('x')
        is_dropout = T.dscalar('is_dropout')
        params = [self.W, self.b1, self.b2]
        hidden = self.activation_function(T.dot(x, self.W)+self.b1)
        hidden_validation = self.activation_function(T.dot(x, self.W)+self.b1)
        arr_n = self.get_mask(self.b1,0.5)
        hidden_tilde = hidden
        hidden_tilde = arr_n * hidden
        output_without_drop = T.dot(hidden_validation,T.transpose(self.W))+self.b2
...
def m_test(data):
    ...
    import scipy.io as sio
    sio.savemat('test.mat',{'weights':W})
